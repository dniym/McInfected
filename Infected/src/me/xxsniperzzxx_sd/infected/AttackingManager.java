...
public class AttackingManager {
	...
	private static void prepareNewZombie(Player newzombie) {
		...
		newzombie.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION,
				20, 2));
	...
	public static void ZombieAttackHuman(Player zombie, Player newzombie) {
		...
		for (Player playing : Bukkit.getServer().getOnlinePlayers())
		{
			...
			if (Main.humans.contains(playing.getName()))
				Methods.rewardPoints(playing, "Survive");
		...
		} else
		{
			...
			{
				if (Infected.isPlayerZombie(zombies) && !(zombies == newzombie))
					Methods.rewardPoints(zombies, "Zombies Infected");
			}
